...
{if $orders || $allow_empty_orders_list_display}
      ...
      <div class="row">
        <div class="col-sm-12" style="padding:0 10px 5px 10px;"><div style="float:left;"> {$lng.lbl_seller_orders_list}</div><div style="float:right;">
...
<thead>
<tr style="background-color: #f2f2f2;">{if $usertype eq 'A'}
	<th width="5"><input type='checkbox' class='select_all' class_to_select='orders_item' /></th>{/if}
	<th width="10%">{if $search_prefilled.sort_field eq "doc_id"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="index.php?target={$current_target}&mode=search&amp;sort=doc_id&amp;sort_direction={if $search_prefilled.sort_field eq 'doc_id'}{if $search_prefilled.sort_direction eq 1}0{else}1{/if}{else}{$search_prefilled.sort_direction}{/if}">Order</a></th>
...
{if $usertype eq 'A'}
	<td width="5"><input type="checkbox" name="doc_ids[{$order.doc_id}]" class="orders_item" /></td>{/if}
	<td width="10%" align="middle"><a href="index.php?target={$current_target}&mode=details&doc_id={$order.doc_id}">#{$order.display_id}</a></td>
...
{else}<b>{include file="main/select/`$doc_status_tpl`.tpl" status=$order.status mode="static"}</b>
{/if}
...
*}
    ...
    <td align="middle" nowrap="nowrap">{$order.date|date_format:$config.Appearance.datetime_format}</td>
...
{/foreach}
{else}
<tr style="background-color: white;"><td colspan="6" align="middle"><b>{$lng.lbl_you_do_not_have_any_orders_yet}</b></td></tr>
{/if}
<tbody>
...
{include file='common/navigation.tpl'}</div> </div>
