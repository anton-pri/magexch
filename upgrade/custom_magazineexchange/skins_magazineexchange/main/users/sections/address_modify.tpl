...
{*
<div class='clear'></div>
{if $address.address_id && !$is_checkout}<label class="save_as"><span>{$lng.lbl_save_as_new}</span> <input type='checkbox' name='{$name_prefix}[as_new]' value='1' /></label>{/if}
</div>
<div class='clear'></div>
*}
<div class="input_field_1" style="padding-top:3px;padding-bottom:3px;">
{if $address.address_id && !$is_checkout}<label class="save_as">{$lng.lbl_add_as_additional_address}</label> <input type='checkbox' name='{$name_prefix}[as_new]' value='1' />{/if}
</div>
</div>
