...
{if !$read_only}
<div class="edit_img">
    <span><input type="button" class="btn btn-green" value="{if $button_name}{$button_name}{else}{$lng.lbl_change_image|strip_tags:false|escape}{/if}" onclick='javascript: popup_image_selection("{$image.in_type|default:$in_type}", "{$image.id}", "{$idtag}","{$tabs}");' /></span>
{if $image.image_id && !$no_delete}
&nbsp;&nbsp;
    <span><input id="{$idtag}_delete" class="btn btn-danger" type="button" value="{$lng.lbl_delete_image|strip_tags:false|escape}" onclick="javascript: {if $delete_js ne ''}{$delete_js|replace:'"':'\"'}{else}self.location='{$delete_url}';{/if}" /></span>
{/if}
</div>
{/if}
