...
<!-- cw@cart_name [ -->
    ...
    <div class="cart_seller_block">
    <ul>
      <li><b>{$lng.lbl_condition}:</b> 
      	{if !$seller_data.is_digital}
			{tunnel func='cw_get_langvar_by_name' via='cw_call' param1="lbl_mag_seller_product_condition_`$seller_data.condition`" param2='' param3=false param4=true}
		{else}
			{$lng.lbl_digital_product|default:'Digital Product'}
		{/if}
	  </li>
	  <li><b>{$lng.lbl_seller_descr}:</b> {$seller_data.comments|nl2br}</li>
      <li><b>{$lng.lbl_seller_info}:</b> <span class="SellerName">{$seller_info.name}</span></li>
...
<!-- cw@cart_qty [ -->
        ...
        {if ($addons.egoods and $product.distribution) || $from_quote || $seller_data.is_digital}{$product.amount}<input type="hidden"{else}<input type="text" size="3"{/if} name="productindexes[{$product.cartid}]" value="{$product.amount}" id="productindexes_{$product.cartid}" {if $use_ajax} onChange="javascript: ajax_update_cart();"{/if} >
<!-- cw@cart_qty ] -->
