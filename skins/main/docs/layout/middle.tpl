...
<tr>
    {foreach from=$layout_data.data.elements item=el}
    <td{if $el eq 'product_id' || $el eq 'sku' || $el eq 'supplier_sku' || $el eq 'amount' || $el eq 'display_net_price' || $el eq 'discount' || $el eq 'display_subtotal'} align="left"{/if} style="border: 1px solid #ddd;">
{if $el eq 'tax'}
