...
{elseif !$multiple}
{select_category category_id=$value|default:$default_category_id assign="category"}
<input type="text" name="{$name}" id="{$id}_catid" value="{$category.category_id}" size="7" style="width: 70px;"{if $disabled} disabled{/if} />
<input type="text" id="{$id}_catname" value="{$category.category}" size="28" class="category {if $class}{$class}{/if}" {if $disabled} disabled{else} readonly{/if} />

<img src="{$ImagesDir}/categories.png" width="16" onclick="javascript: return cw_show_categories('{$category.category_id}', '{$id}', '');" id="{$id}_link" class="pointer" />
...
<input type="text" value="{$category.category_id}" size="7" style="width: 70px;" disabled />
<input type="text" value="{$category.category|escape}" style="width: 245px;" size="28" disabled /><br/>
{/foreach}
{else}
{$lng.lbl_none}
{/if}
...
<tr>
    <td id="{$id}_box_1"><input type="text" name="{$name}" id="{$id}_catid_0" value="" style="width: 70px;" size="7"{if $disabled} disabled{/if} />&nbsp;</td>
    <td id="{$id}_box_2"><input type="text" name="name_{$name}" id="{$id}_catname_0" value="" class="category" size="28"{if $disabled} disabled{else} readonly{/if} />&nbsp;</td>
    <td id="{$id}_box_3">
