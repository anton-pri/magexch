...
<table class="header orders" width="100%">
<tr>
...
{include file="main/select/`$doc_status_tpl`.tpl" status=$order.status mode="select" name="order_status[`$order.doc_id`]" extra="class='short'"}
{else}
	...
	{if $current_area eq 'A'}
		<td>
			...
			<span class="order_tooltip" title="{$order.customer_notes}">
				<img alt="{$lng.lbl_customer_notes}" src="{$SkinDir}/images/icon_info_small_grey.gif" height="18px" width="18px" />
			</span>
			{/if}
		</td>
		<td>
			...
			<span class="order_tooltip" title="{$order.notes}">
				<img alt="{$lng.lbl_notes}" src="{$SkinDir}/images/icon_info_small_red.gif" height="18px" width="18px" />
			</span>
			{/if}
		</td>
	{/if}
...
<div class="input_field_easy_1_1 right_al">
...
<div class="buttons">
{include file='buttons/button.tpl' href="javascript: cw_submit_form('process_order_form', 'update');" button_title=$lng.lbl_update acl=$page_acl}
{include file='buttons/button.tpl' href="javascript: cw_submit_form('process_order_form', 'delete');" button_title=$lng.lbl_delete_selected acl=$page_acl}
{include file='buttons/button.tpl' href="javascript: cw_submit_form_check('print', 'invoices');" button_title=$lng.lbl_print_selected}
...
{if $usertype eq "A"}
  ...
  {else}
    {assign var='doc_status_tpl' value="doc_status"}
  {/if}
  {include file='buttons/button.tpl' href="javascript: cw_submit_form('process_order_form', 'mass_update');" button_title=$lng.lbl_update_statuses_to}
