...
{/if}
    ...
    <td width="200"><a href="index.php?target={$current_target}&mode=modify&user={$user.customer_id}" title="{$lng.lbl_modify_profile|escape}">{$user.email}</a></td>
	<td><a href="index.php?target={$current_target}&mode=modify&user={$user.customer_id}">{*$user.customer_id|user_title:$user.usertype*}{if $user.firstname or $user.lastname}{$user.firstname} {$user.lastname}{else}{$lng.lbl_anonymous_customer}{/if}</a></td>
    <td>{$user.phone}</td>
    <td>{$user.zipcode}</td>	
...
<div class="buttons">
{include file='buttons/button.tpl' button_title=$lng.lbl_delete_selected onclick="javascript: document.forms.process_user_form.action.value='delete'; submitFormAjax('process_user_form');" acl=$page_acl}
&nbsp;&nbsp;&nbsp;&nbsp;
{include file='buttons/button.tpl' button_title=$lng.lbl_export_email_addresses  href="index.php?target=`$current_target`&mode=search&action=export_emails" acl=$page_acl}
</div>
{/if}
