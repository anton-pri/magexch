...
<li href='index.php?target=user&mode=addresses&action=load&address_id={$address.address_id}' id='address_label_{$address.address_id}' {if $address.address_id eq $address_id}class='active'{/if}>
    ...
    {if !$address.main} | <a href='index.php?target=user&mode=addresses&action=set_main&address_id={$address.address_id}'>Set as {$lng.lbl_billing_address}</a>{/if}
    {if !$address.current} | <a href='index.php?target=user&mode=addresses&action=set_current&address_id={$address.address_id}'>Set as {$lng.lbl_shipping_address}</a>{/if}
    </div>
</li>
{/foreach}
