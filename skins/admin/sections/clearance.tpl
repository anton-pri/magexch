...
<form action="index.php?target={$current_target}" method="post" name="section_clearance_form">
...
<tr>
    <th align="center" style="width:10px;"><input type='checkbox' class='select_all' class_to_select='clearance_item' /></th>
...
{if $clearance}
{foreach from=$clearance item=sec}
<tr{cycle values=", class='cycle'"}>
    <td align="center"><input type="checkbox" name="data[{$sec.id}][del]" value="Y" class="clearance_item" /></td>
    <td>
...
{/foreach}
<tr><td colspan="6">{include file='admin/buttons/button.tpl' href="javascript:document.section_clearance_form.submit();" button_title=$lng.lbl_update_delete style="btn-danger"}</td></tr>
{else}
<tr>
...
<tr>
    <td colspan="2" >
		{product_selector name_for_id='newproduct_id' name_for_name='newproduct' prefix_id='section_clearance_form' hide_id_field=1 form='section_clearance_form'}
    </td>
    ...
    <td align="center"><input type="checkbox" name="data[0][home_page]" value="1" /></td>
</tr>
{/if}
</table>
<div class="buttons">{include file='admin/buttons/button.tpl' href="javascript:cw_submit_form(document.section_clearance_form, 'add');" button_title=$lng.lbl_add acl='__1207' style="btn-green push-20"}</div>
</div>

</form>
{/capture}
{include file="admin/wrappers/block.tpl" content=$smarty.capture.section extra='width="100%"' title=$lng.lbl_clearance}
