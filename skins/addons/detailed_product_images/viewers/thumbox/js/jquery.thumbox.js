/** 
  ...
  *
  *  @author     Jose Raimundo <sputinykster@gmail.com>
  *  @version    1.1
...
(function($,doc){
    $.fn.thumbox = function(settings) {
        ...
        $all.each(function(k,matched){
            ...
            if(objList.size() == 1){
                $(objList).data('settings', settings).bind('click', function(e){
                    $(this).blur();
                    e.preventDefault();
                    _wasAborted = false;
                    img   = $(this).find('img');
                    ...
                    openImageFromPage(big, label, coord, $(this).data('settings'));
                })
            } else {
        ...
        showDock = function(objs, params, autoopen){
            ...
            _wasAborted = false;
            showLoader();
            maxThumbW = (_plParams.maxThumbWidth == undefined) ? 1000000 : _plParams.maxThumbWidth;
            maxThumbH = (_plParams.maxThumbHeight == undefined) ? 1000000 : _plParams.maxThumbHeight;
            cls       = false;
            ...
            if(_plParams.wheelNavigation){
                attatchWheelEvent();
            }
            
            if(_plParams.keyboardNavigation){
            ...
            _isSingleImg     = false;
            tTop = (_plParams.dockPosition == 'top') ? ($(document).scrollTop()+10) : ($(window).height()+$(document).scrollTop()-$('#thumbox-dock').height()-10);
            $('#thumbox-dock').css({
                'top':tTop,
                'width':wmax+40,
                left:($(window).width()/2)-(wmax/2)-20
            }).hide().fadeIn(_plParams.overlaySpeed, function(){
                unlockHandlers();
                if(autoopen != undefined) {
                    goToPage(Math.ceil(autoopen/_plParams.thumbs), autoopen)
                }
            })
        };
        ...
        var keyboardNav = function(evt){
            ...
            if((!_plParams) ||(!_plParams.keyboardNavigation) || (jQuery.inArray(evt.keyCode, [39,40,13,32,37,38,8,27,36,35]) == -1)) return;
            evt.preventDefault();
            if(isLocked()) return;
            switch (evt.keyCode) {
                case 39:
        ...
        var setPlParams = function(listValues){
            _plParams = listValues;
            if($.easing.def == undefined) {
                _plParams.openImageEffect  = 'linear';
                _plParams.closeImageEffect = 'linear';
                _plParams.scrollDockEffect = 'linear';
            }
        }
        ...
        var openImageFromDock = function(imgindex){
            ...
            lockHandlers();
            page = Math.ceil(imgindex/_plParams.thumbs);
            if(_page != page){
        ...
        var showImage = function(cord, urlImg, label, imgindex){
            ...
            var timeOutThumbox = setInterval(function(){
                if(_clockTimeOut > _plParams.timeOut){
            ...
            objImagePreloader.onload  = function(){
                ...
                $(img).css(cord).show().animate({
                    ...
                    width:imgWidth,
                    height:imgHeight
                }, {"easing": _plParams.openImageEffect, duration:_plParams.zoomSpeed, complete:function(){
                    ...
                    //label na imagem
                    if(_plParams.showLabel && (label != '')){
                        $('body').append('<div id="thumbox-label"></div>');
                        $('#thumbox-label').html(label);
                        topPosLabel = (_plParams.labelPosition == 'bottom') ? pos.top+imgHeight+10 : pos.top-25-$('#thumbox-label').height();
                        $('#thumbox-label').css({
                            top:topPosLabel,
        ...
        var showOverlay = function(){
            $('body').append('<div id="thumbox-overlay" style="display:none"></div>').css({'overflow':'hidden'});
            $('#thumbox-overlay').bind('click', function(){
                ...
                'background':_plParams.overlayColor
            }).fadeTo(_plParams.overlaySpeed, _plParams.overlayOpacity);
        }
	
        var closeOverlay = function(){
            $('#thumbox-overlay').fadeOut(_plParams.overlaySpeed, function(){
        ...
        var goToPage = function(goTo, autoopen){
            ...
            if(!move){
                ...
                $('.thumbox-pages').animate({
                    left:walk
                }, {easing:_plParams.scrollDockEffect, duretion:_plParams.scrollSpeed, complete:function(){
                    if(autoopen != undefined){
                        openImageFromDock(autoopen)
                    } else {
                        unlockHandlers();
        ...
        var closeImage = function(open, toCloseDock){ 
            if($('#thumbox-image').size()){
                ...
                $('#thumbox-image').animate(coord, {easing:_plParams.closeImageEffect, duration:_plParams.zoomSpeed, complete:function(){
        ...
        var closeDock = function(){
            ...
            if ($('#thumbox-image').size()) {
                ...
                if($('#thumbox-dock').size()){
                    hideControls();
                    $('#thumbox-dock').fadeOut(_plParams.overlaySpeed, function(){
                        hideLoader();
