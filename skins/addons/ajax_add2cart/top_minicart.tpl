...
{if $cart.products}
<form id="minicartform" name="minicartform" method="post" action="{$current_location}/index.php?target=cart&get_top_minicart=1">
...
<div class='minicart_item {cycle values="cycle,"} {if $smarty.foreach.minicart.first}first{/if}'>
    ...
    <div class='minicart_delete' width='4%'>
        <a class='top_minicart_control' href='javascript: void(0);' onclick="ajaxGet('{$current_location}/index.php?target=cart&action=delete&productindex={$minicart_prod.cartid}&get_top_minicart=1', 'top_minicart');"></a>
    </div>
    <!-- cw@minicart_delete ] -->
...
{*    <a class="top_minicart_link float-left" href='javascript: void(0);' onclick="blockElements('top_minicart',true); submitFormAjax('minicartform',$.unblockUI);">Update</a>*}
    <a class="top_minicart_link float-left" href='{$current_location}/index.php?target=cart'>View cart</a>
    <a class='minicart_control top_minicart_link float-right' href='javascript: void(0);' onclick="ajaxGet('{$current_location}/index.php?target=cart&action=clear_cart', 'top_minicart', microcart_content_hide);">Clear cart</a>
    <div class='clear'></div>
    <a class="top_minicart_link checkout" href='{$current_location}/index.php?target=cart&mode=checkout'>Checkout <i class="icon-chevron-right"></i></a>
