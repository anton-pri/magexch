...
function setDatahubItem () {
    ...
    if (current_sel_item) {
        ...
        var url = 'index.php?target=dh_save_match&table_id='+buffer_table_id+'&key=9999999&manual_sel_id='+current_sel_item;

        if (window.parent.is_interim)  
            url += '&is_interim=1';

        ajaxGet(url);
        setTimeout(function(){ window.parent.hm('item_select_popup'); }, 1000);
...
$(document).ready(function() {
    ...
    $('#main_data_table tbody').on( 'click', 'tr', function () {
        if ( $(this).hasClass('selected') ) {
            ...
            dh_current_choice_text = dh_clean(mdt_data[3]) + dh_clean(mdt_data[4]) + dh_clean(mdt_data[5]) + dh_clean(mdt_data[6]) + dh_clean(mdt_data[8]) + dh_clean(mdt_data[10]) + dh_clean(mdt_data[12]) + dh_clean(mdt_data[13]) + dh_clean(mdt_data[14]);
            $('#dh_current_choice_text').html('<b title=\''+current_sel_item+'\'>Selected Item: #' +mdt_data[1] + '</b> ' + dh_current_choice_text);
        }
    } );
...
{include file='buttons/button.tpl' button_title=$lng.lbl_choose href="javascript: setDatahubItem();  void(0);" style='btn-green'}&nbsp;{include file='buttons/button.tpl' button_title=$lng.lbl_close href="javascript: closeDatahubItemsPopup();  void(0);" style='btn-danger'}</div>
<br />
<br />
