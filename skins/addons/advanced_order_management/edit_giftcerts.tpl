...
<form action="index.php?target={$postfix}" method="post" name="editgc_form">
<input type="hidden" name="mode" value="edit" />
<input type="hidden" name="action" value="update_giftcerts" />
<input type="hidden" name="show" value="giftcerts" />
<input type="hidden" name="doc_id" value="{$doc_id}" />

{include file="common/subheader.tpl" title=$lng.lbl_gift_certificates}
...
<tr>
	...
	<a href="index.php?target=giftcerts&mode=modify_gc&amp;gc_id={$giftcert.gc_id}" target="viewgc{$giftcert.gc_id}">{$lng.lbl_gc_id} #{$giftcert.gc_id}</a>
{if $giftcert.deleted}
...
<tr>
	<td><input type="checkbox" name="giftcert_details[{%gc.index%}][delete]" value="{$giftcert.gc_id}" /></td>
...
<tr>
	...
	<td valign="top">{if $giftcert.deleted}{include file='common/currency.tpl' value=$giftcert.amount}{else}<input type="text" name="giftcert_details[{%gc.index%}][amount]" size="15" maxlength="15" value="{$giftcert.amount|formatprice}" />{/if}</td>
	<td valign="top">{include file='common/currency.tpl' value=$orig_giftcert.amount}</td>
</tr>
...
<tr>
	<td colspan="3"><br />
{include file="buttons/update.tpl" href="javascript:cw_submit_form(document.editgc_form);"}
<br /><br />
	</td>
</tr>
{else}
...
{/capture}
{include file="common/section.tpl" title=$lng.lbl_aom_edit_giftcerts_title|substitute:"doc_id":$order.display_id content=$smarty.capture.section extra='width="100%"'}
