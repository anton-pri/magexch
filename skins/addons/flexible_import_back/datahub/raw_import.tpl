...
{capture assign='dh_main_display_title'}{$lng.lbl_datahub_current_count_of_buffer_items|default:'Current count of items in import hub buffer table'}: {$buffer_lines_count|default:0}{/capture}
...
<tr>
<th>{$lng.lbl_name}</th>
<th>{$lng.lbl_active_reccuring_task|default:'Active recurring task'}</th>
...
<td>{if $profile.recurring_import_path ne ''}{$profile.recurring_import_path}{else}&nbsp;{/if}</td>
...
<td><span {$row_type_color}>{$le.event_message|escape:'html'}</span></td>
</tr>
{/foreach}
</table>
{/capture}
