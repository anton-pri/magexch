{include file="addons/flexible_import/flexible_import_menu.tpl" active="4"}
...
<!--
var all_tr_elems = [{foreach from=$found_items item=itemvals name=products}{$itemvals.ID}{if !$smarty.foreach.products.last},{/if}{/foreach}];
{literal}
...
<tr>
<td><input type="checkbox" name="print_items[{$itemvals.ID}]" value="1" class="print_tag_item" /></td>
<td>
<input type="checkbox" name="hide_items[{$itemvals.ID}]" value="1" {if $itemvals.hiddenID ne ''}checked="checked"{/if} class="hide_tag_item"/>
<input type="hidden" name="process_hide_items[{$itemvals.ID}]" value="1" />
</td>
{foreach from=$display_cols item=colval key=colname}
<td onclick="javascript: toggle_ratings('{$itemvals.ID}', '');">&nbsp;{if $colname eq 'Regular Price'}${$itemvals.$colname|number_format:2:".":","}{else}{$itemvals.$colname}{/if}&nbsp;</td>
{/foreach}
</tr>
<tr id="ratings_edit_{$itemvals.ID}" style="display:none;"><td colspan="13">
<table width="100%" >
<tr>
{foreach from=$rating_cols key=colgrpid item=colgrp}<td width="25%">
{assign var="keyname" value=$colgrp.0}
{$keyname}:&nbsp;<br />
<input type="text" name="ratings[{$itemvals.ID}][{$keyname}]" style="width:95%" value="{$itemvals.$keyname}"><br />
{assign var="keyname" value=$colgrp.1}
{$keyname}:&nbsp;<br />
<textarea name="ratings[{$itemvals.ID}][{$keyname}]" style="width:95%; height:110px">{$itemvals.$keyname}</textarea>
...
<script type="text/javascript">
var printwindow = window.open("index.php?target=datahub_rnt_tag_print&type={if $items2print_1 eq 'Y'}print1{elseif $items2print_2 eq 'Y'}print2{/if}", "PrintTags");
printwindow.focus();
</script>
{/if}
