...
<tr>
  ...
  <td align='left'>
    {if $field_def.edit_type eq 'image'}
      <img id='old_val_{$table_id}_{$htmlfname}' src="{$buffer_item.image.filename|default:'/images/no_image.jpg'}" style="max-width:100px;max-height:100px;text-align:center;"/>
    {else}
  ...
  <td align='left'>
   ...
   {else}
       ...
       {elseif $field_def.edit_type eq 'image'}
         <img id='edit_buffer_item_{$table_id}_{$htmlfname}' src="{$buffer_item.image.filename|default:'/images/no_image.jpg'}" style="max-width:100px;max-height:100px;"/>
         <input type="hidden" name='edit_buffer_item[{$table_id}][{$fname}]' src="{$buffer_item.image.filename|default:'/images/no_image.jpg'}" />
       {else}
...
function dh_manual_copy_field(bfield) {
    if (bfield == 'image') {
        ...
        $('#edit_buffer_item_'+table_id+'_'+bfield).val($('#merge_src_'+table_id + '_' + bfield).html());
    }
}
...
{if $bfielddata.merge_cond eq 'A'}
  $('#edit_buffer_item_'+table_id+'_{$htmlfname}').val($('#merge_src_'+table_id + '_{$htmlfname}').html());
{else}
  {if $bfielddata.merge_cond eq 'E'}
    if ($('#merge_src_'+table_id + '_{$htmlfname}').html() != '' && $('#edit_buffer_item_'+table_id+'_{$htmlfname}').val() == '') 
      $('#edit_buffer_item_'+table_id+'_{$htmlfname}').val($('#merge_src_'+table_id + '_{$htmlfname}').html());
  {/if}
{/if}
{/foreach}
...
function dh_field_reset(bfield) {
    if (bfield == 'image') {
        ...
        $('#edit_buffer_item_'+table_id+'_'+bfield).val($('#old_val_'+table_id + '_' + bfield).html());
    }
}
