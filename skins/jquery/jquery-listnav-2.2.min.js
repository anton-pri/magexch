...
if($.cookie&&opts.cookieName!==null){var cookieLetter=$.cookie(opts.cookieName);if(cookieLetter!==null)opts.initLetter=cookieLetter}if(opts.initLetter!==""){firstClick=true;$("."+opts.initLetter.toLowerCase(),$letters).slice(0,1).click()}else if(opts.includeAll)$(".all",$letters).addClass("ln-selected");else for(var i=opts.includeNums?0:1;i<letters.length;i++)if(counts[letters[i]]>0){firstClick=true;$("."+letters[i],$letters).slice(0,1).click();break}}
...
$letters).addClass("ln-disabled")}function addNoMatchLI(){$list.append('<li class="ln-no-match listNavHide">'+opts.noMatchText+"</li>")}function getLetterCount(el){if($(el).hasClass("all"))return allCount;else{var count=counts[$(el).attr("class").split(" ")[0]];return count!==undefined?count:0}}function bindHandlers(){if(opts.showCounts){$wrapper.mouseover(function(){});$(".ln-letters a",$wrapper).mouseover(function(){var left=$(this).position().left,width=$(this).outerWidth()+
...
html.push('<a class="'+letters[i]+'" href="#">'+(letters[i]==="-"?"...":letters[i].toUpperCase())+"</a>")}return'<div class="ln-letters">'+html.join("")+"</div>"}init()})};$.fn.listnav.defaults={initLetter:"",includeAll:true,incudeOther:false,includeNums:true,flagDisabled:true,removeDisabled:false,noMatchText:"No matching entries",showCounts:true,cookieName:null,onClick:null,prefixes:[],filterSelector:""}})(jQuery);