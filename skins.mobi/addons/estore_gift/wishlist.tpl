...
{if $product.product_options}
    ...
    {/if}
{/if}
{if $addons.Product_Configurator and $product.product_type eq 3}
    {include file='addons/Product_Configurator/pconf_customer_cart.tpl' products=$product.subproducts main_product=$product}
{/if}

{if $addons.subscriptions and $product.catalogprice}
    {include file='addons/subscriptions/subscription_priceincart.tpl' product=$product}
{else}
...
<input type="text" size="3" name="quantity" value="{$product.amount}" />
{else}
...
<input type="hidden" name="action" value="entire_list" />
	{$lng.lbl_send_entire_wishlist}: <input type="text" size="18" name="friend_email" />&nbsp;
	{include file='buttons/button.tpl' href="javascript:cw_submit_form('sendall_form')"}
</form>
{include file='buttons/button.tpl' button_title=$lng.lbl_wl_clear href="index.php?target=`$current_target`&mode=wishlist&action=wlclear&event_id=`$event_id`"}
{/if}
