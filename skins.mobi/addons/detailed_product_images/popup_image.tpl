{if $images}
	...
	<a href="#" id="product-image-anchor" onclick="showImages(); return false;">
...
{* Mobile
	...
	{foreach from=$images key=k item=dpi_image name=dimage}
		...
		<a class="dpi-thumb" rel="{$rel_value}" href="{$images[$k].tmbn_url}" title="{$dpi_image.alt|escape:'html'}"><img rel="{$dpi_image.image_id}" alt="{$dpi_image.alt|escape:'html'}" src="{$images[$k].dpi_thumbnail_url}" height="{$images[$k].dpi_thumb_y}" width="{$images[$k].dpi_thumb_x}" title="{$dpi_image.alt|escape:'html'}" /></a>
	{/foreach}
		</ul>
...
*}
	...
	{foreach from=$images key=k item=dpi_image name=dimage}
		productImages[{$dpi_image.image_id}] = "{$dpi_image.dpi_primage_url}";
	{/foreach}	
	...
	{literal}
		...
		if (is_zoomifier != false) {
			...
			_first_thumb.attr('href', productImgObj.attr('src'));
			_new_index = productImages.length;
			productImages[_new_index] = productImgObj.attr('src');
			_first_thumb.find('img:first').attr('src', productImgObj.attr('src')).attr('rel', _new_index);
