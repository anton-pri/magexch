  ...
  function get_max_width(&$context) {
    ...
    if ($wc && $wc->isConstant()) {
      ...
      for ($i=0; $i<$this->cols_count(); $i++) {
        ...
        $min = 0;
        if ($cwc) $min = $cwc->apply_inverse($widths[$i], $base_width);
        $min = min($min, $this->parent->get_available_width($context) - $this->_get_hor_extra());
        $width = max($width, $min);
      };
...
//       break;
  ...
  function reflow_static_normal(&$parent, &$context) {
    ...
    if ($border->left) $plus += $border->left->get_width();
    if ($padding->left) $plus += $padding->left->value;

    $this->put_left($plus);
    
    // Note that top margin already used above during maring collapsing
    $mins = $parent->_current_y;
    if ($border->top) $mins -= $border->top->get_width();
    if ($padding->top) $mins -= $padding->top->value;
    $this->put_top($mins);
