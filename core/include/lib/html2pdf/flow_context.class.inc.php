  ...
  function float_left_x($x, $y) {
    ...
    for ($i=0; $i<$size; $i++) {
      ...
      if ($float->getCSSProperty(CSS_FLOAT) == FLOAT_LEFT) {
        ...
        if ($float->get_top_margin() + EPSILON >= $y && $float->get_bottom_margin() < $y) {
...
# kornev, full function redesign - speed up
    $floats =& $this->current_floats();
    $float_bottoms = array();
    $size = count($floats);
    ...
    $mayBeExpanded = $parent->mayBeExpanded();

    for($i=0; $i < $size; $i++) {
      $x  = $this->float_left_x($parent_left, $y);
      ...
      if ($parent_right + EPSILON >= $x + $width || $mayBeExpanded) return;
