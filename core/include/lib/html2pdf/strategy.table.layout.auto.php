...
class StrategyTableLayoutAuto {
  ...
  function table_columns_fit(&$table, $width, &$context) {
    ...
    if (!$cellpadding) $cellpadding = CSSCellPadding::default_value();
    $cellspacing = $table->getCSSProperty(CSS_HTML2PS_CELLSPACING);
    if (!$cellspacing) $cellspacing = CSSCellSpacing::default_value();

    for ($i=0; $i<count($minw); $i++) {
      $cwc = $table->get_cwc($i);
      if (!$cwc) $cwc =  new WCNone;
    ...
    if (!$wc) $wc = CSSCompositeWidth::default_value();
    if (!$wc->isNull()) {
