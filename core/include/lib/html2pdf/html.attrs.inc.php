<?php
// $Header: /var/cvs/arsgate/client_cpanel/include/html2pdf/html.attrs.inc.php,v 1.3 2008-07-04 16:28:38 cvs Exp $
...
# kornev, optimization
    ...
    if ($g_tag_attrs[$tagname])
    foreach ($g_tag_attrs[$tagname] as $attr => $fun) {
        if ($root->has_attribute($attr)) {
            $fun = $fun.$suffix;
            $fun($root, $pipeline);
        }
