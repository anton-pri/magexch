...
class GenericFormattedBox extends GenericBox {
  ...
  function _get_collapsable_top_margin_internal() {
    ...
    if (($border->top && $border->top->get_width() > 0) ||
       ($padding->top && $padding->top->value > 0)) {
      return 0;
    };
  ...
  function _get_collapsable_bottom_margin_external() {
    ...
    while (!is_null($current_box) && 
      ...
      if (($border->bottom && $border->bottom->get_width() > 0) ||
          ($padding->bottom && $padding->bottom->value > 0)) {
        $current_box = null;
      } else {
  ...
  function collapse_margin_bottom(&$parent, &$context) {
    ...
    if (!is_null($last) && 
        ...
        (!$parent_border->bottom || $parent_border->bottom->get_width() == 0)  && // Parent have NO bottom border AND
        (!$parent_padding->bottom || $parent_padding->bottom->value == 0))  {      // Parent have NO bottom padding AND
     ...
     */
    if ($is_last &&
        (($parent_border->bottom && $parent_border->bottom->get_width() > 0) ||
         ($parent_padding->bottom && $parent_padding->bottom->value > 0))) {
...
if ($wc)
  ...
  function get_extra_bottom() {
    $border = $this->getCSSProperty(CSS_BORDER);
    if ($border->bottom)
        return $this->get_margin_bottom() + $border->bottom->get_width() + $this->get_padding_bottom();
    return $this->get_margin_bottom() + $this->get_padding_bottom();
  }
  ...
  function get_extra_left() {
    ...
    if ($border->left) 
        return $this->get_margin_left() + $border->left->get_width() + $this->get_padding_left();
    return $this->get_margin_left() + $this->get_padding_left();
  }

  function get_extra_right() {
    $border = $this->getCSSProperty(CSS_BORDER);
    if ($border->right) return $this->get_margin_right() + $border->right->get_width() + $this->get_padding_right();
    return $this->get_margin_right() + $this->get_padding_right();
  }

  function get_extra_top() {
    $return = 0;
    $border = $this->getCSSProperty(CSS_BORDER);
    if ($border->top) $return += $border->top->get_width();

    return  $return + $this->get_margin_top() + $this->get_padding_top();
  }
  ...
  function get_left_border() { 
    ...
    $sum = $this->get_left();
    if ($padding->left) $sum -= $padding->left->value;
    if ($border->left) $sum -= $border->left->get_width();
  ...
  function get_right_border() { 
    ...
    $sum = $this->get_left() + $this->get_width() + $padding->right->value;
    if ($border->right) return $sum + $border->right->get_width(); 
    return  $sum;
  }
  ...
  function get_top_border()     { 
    ...
    if ($border->top) return $this->get_top_padding() + $border->top->get_width();  
  ...
  function get_bottom_border()  { 
    $border = $this->getCSSProperty(CSS_BORDER);
    if ($border->bottom)
        return $this->get_bottom_padding()  - $border->bottom->get_width();  
