...
class WhitespaceBox extends TextBox {
  ...
  function reflow_text(&$driver) {
    ...
    $this->width = $this->height * WHITESPACE_FONT_SIZE_FRACTION;
    if ($letter_spacing) $this->width += $letter_spacing->getPoints();
    if ($word_spacing) $this->width += $word_spacing->getPoints();
