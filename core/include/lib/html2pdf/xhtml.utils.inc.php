...
function fix_attrs_tag($tag) {
  ...
  foreach ($attrs as $key => $value) {
    ...
    if (HTML2PS_XMLUtils::valid_attribute_name($key))
        $str .= ' '.$key.'="'.(strpos($value,'"') !== false?addslashes($value):$value).'"';
  };
...
# kornev, no sense - we are sure that it's fine
/*
...
*/
  ...
  while (preg_match("/(<\/?)([^>^\s]*[A-Z]+[^>^\s]*)([\s|>])/Us", $html, $matches))
      $html = preg_replace("#$matches[1].$matches[2].$matches[3]#is", $matches[2].strtolower($matches[3]).$matches[4], $html);
  return $html;
};
