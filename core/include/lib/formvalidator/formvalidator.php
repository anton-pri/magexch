...
/*
    ...
    CUSTOM CHANGES IN CW:
    - ValidateForm() accept parameter - array of the form fields. Default is $_POST or $_GET
    - "varname" validator
    - "url" validator
...
class FormValidator 
{
	...
	function ValidateForm(&$form_variables = null)
	{
		$bret = true;
            ...
            case 'varname':
                        {
                            ...
                            $bret &= $this->test_datatype($input_value[0],"[0-9]");
                            if(false == $bret)
                            {
                                $default_error_message = sprintf(E_VAL_ALNUM_CHECK_FAILED,$variable_name);
                            }
			...
			case 'url':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								...
								if(false == $bret)
								{
									$default_error_message = 'Invalid URL';
								}
							}
							break;
						}
