...
function smarty_function_html_select_date($params, &$smarty) {
	...
	if ($display_months) {
		...
		for ($i = 1; $i <= 12; $i++) {
			if (!$is_native) {
				if ($is_fullname) {
					$month_names[$i] = str_replace("%B", strftime(cw_get_langvar_by_name("lbl_month_fullname_".$i), mktime(0, 0, 0, $i, 1, 2000)), $month_format_orig);
				}
				if ($is_abbr) {
					$month_names[$i] = str_replace("%b", strftime(cw_get_langvar_by_name("lbl_month_abbr_".$i), mktime(0, 0, 0, $i, 1, 2000)), $month_format_orig);;
				}
			}
