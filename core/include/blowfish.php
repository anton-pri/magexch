...
class ctBlowfish {
	...
	function bf_enc($ll, $r, $n) {
		$ll = cw_xor($ll, $this->pk[$n]);

        $b1 = $this->sk[cw_and(cw_rshift($r, 24), 0xff)];
        $b2 = $this->sk[256 + cw_and(cw_rshift($r, 16), 0xff)];
        $b3 = $this->sk[512 + cw_and(cw_rshift($r, 8), 0xff)];
        $b4 = $this->sk[768 + cw_and($r, 0xff)];

		return cw_xor($ll,  cw_xor(($b1 + $b2), $b3) + $b4);
	}

	function bf_encrypt($l, $r) {
		$l = cw_xor($l, $this->pk[0]);
		for ($i = 1; $i < 17; $i += 2) {
			$r = $this->bf_enc($r, $l, $i);
			$l = $this->bf_enc($l, $r, $i + 1);
		}

		$r = cw_xor($r, $this->pk[17]);
		return array($r, $l);
	}
	function bf_decrypt($l, $r) {
		$l = cw_xor($l, $this->pk[17]);
		for ($i = 16; $i > 0; $i -= 2) {
			$r = $this->bf_enc($r, $l, $i);
			$l = $this->bf_enc($l, $r, $i - 1);
		}
		$r = cw_xor($r, $this->pk[0]);
		return array($r, $l);
	}
	...
	function int2asc($int) {
		return chr(cw_rshift($int, 24)).chr(cw_and(cw_rshift($int, 16), 0xff)).chr(cw_and(cw_rshift($int, 8), 0xff)).chr(cw_and($int, 0xff));
	}
	...
	function bf_set_key($key) {
		...
		for ($i = 0; $i < 18; $i ++) {
			$this->pk[$i] = cw_xor($this->pk[$i], $this->asc2int(substr($key, $i * 4, 4)));
		}
	...
	function mcrypt($data, $key) {
		...
		return cw_str2hex($encrypted_data);
	}
	...
	function mdecrypt($data, $key) {
        ...
        $decrypted_data = mdecrypt_generic($td, cw_hex2str($data));
        mcrypt_generic_deinit($td);
        mcrypt_module_close($td);
...
function cw_bf_psc($prop, $key) {
	global $blowfish;

	return cw_bf_asc($blowfish->$prop, $key);
}
...
#
function cw_bf_check_env() {
	global $bf_wrapper, $blowfish;

	if (defined('APP_SESSION_START') && function_exists('cw_session_register')) {
		$bf_mode = &cw_session_register("bf_mode");
	}
