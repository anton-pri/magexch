...
function __curl_headers() {
    ...
    if (count($args) == 1) {
        $return = '';
        if ($args[0] == true) $return = $curl_headers;
        $curl_headers = '';
        return $return;
    }

    if (trim($args[1]) != '') $curl_headers .= $args[1];
    return strlen($args[1]);
}

function cw_https_request_libcurl($method, $url, $data="", $join="&", $cookie="", $conttype="application/x-www-form-urlencoded", $referer="", $cert="", $kcert="", $headers="", $timeout = 0) {
	global $config;

	if (!function_exists('curl_init'))
		return array("0","HTTPS: libcurl is not supported");
...
//	__curl_headers(false);
	...
	curl_close ($ch);
	if ($errno)
		return array("0","HTTPS: libcurl error($errno): $error");
	return array($curl_headers, $body);//array(__curl_headers(true), $body);
}
