...
//-------------
        ...
        if (empty($tags_sortby) || in_array($tags_sortby,array('name','product'))) {
            ...
            $dh_rnt_search_params['sortby'] = "$tables[datahub_main_data].catalog_id"; 
    ...
    if ($action == "search") {
        ...
        $items_prices = $print_items_prices;
        ...
        if (is_array($print_items)) {
            ...
            if (count(array_keys($print_items))) {
                cw_header_location("index.php?target=datahub_rnt&mode=print&sales_tag=".$request_prepared['sales_tag']);
                exit;
            }  
        ...
        if ($ratings) { 
            ...
            foreach ($ratings as $item_id => $ratings_data) {
                ...
                db_query("update $tables[datahub_main_data] set ".implode(", ", $update_fields)." where catalog_id = '$item_id'");
            }
        }
...
if (!empty($search_for) || $dh_rnt_search_params['live_only'] || !empty($dh_rnt_search_params['empty_rating_search']) || !empty($dh_rnt_search_params['empty_review_search'])) {
    ...
    if (!empty($search_for)) { 
        ...
        $search_conditions[] = "$tables[datahub_main_data].catalog_id in ('".implode("','", $search_items_arr)."')";
    }

    if ($dh_rnt_search_params['live_only']) {
        $live_only_inn_join = " inner join $tables[products] on $tables[products].product_id = $tables[datahub_main_data].catalog_id AND $tables[products].status=1";
    ...
    if ($dh_rnt_search_params['hidden_only']) {
        $hidden_join = " inner join $tables[datahub_rnt_hidden_item] on $tables[datahub_rnt_hidden_item].hiddenID = $tables[datahub_main_data].catalog_id ";
    } else {
        $hidden_join = " left outer join $tables[datahub_rnt_hidden_item] on $tables[datahub_rnt_hidden_item].hiddenID = $tables[datahub_main_data].catalog_id ";
    ...
    $items_count_res = cw_query("select count(*) as count from $tables[datahub_main_data] $live_only_inn_join $hidden_join where $tables[datahub_main_data].catalog_id != '' and ".implode(" and ", $search_conditions));  
...
//--navigation.php
    $items_limit = " LIMIT ".max(0,((min($page,1)-1)*$objects_per_page)).", $objects_per_page";

    if (!empty($search_for)) {
        $qry = "SELECT ".implode(", ", $select_fields).", p.`Regular Price` as 'Regular_Price', p.MSRP, p.`Item Description` as 'Item Description' FROM $tables[datahub_main_data] $live_only_inn_join $hidden_join LEFT JOIN pos p ON p.`Alternate Lookup`=$tables[datahub_main_data].catalog_id WHERE $tables[datahub_main_data].catalog_id != '' AND ".implode(" AND ", $search_conditions).$sort_by_qry.$items_limit;

    } else {
        db_query("drop table if exists pos_ALU");
        db_query("create temporary table pos_ALU like pos"); 
        db_query("insert into pos_ALU select * from pos"); 
        db_query("alter table pos_ALU add column ALU int(11) not null default 0"); 
        db_query("update pos_ALU set ALU = cast(`Alternate Lookup` as signed)"); 
        db_query("alter table pos_ALU add index _alu (ALU)");

        $qry = "SELECT ".implode(", ", $select_fields).", p.`Regular Price` as 'Regular_Price', p.MSRP, p.`Item Description` as 'Item Description' FROM $tables[datahub_main_data] $live_only_inn_join $hidden_join LEFT JOIN pos_ALU p ON p.`ALU`=$tables[datahub_main_data].catalog_id WHERE $tables[datahub_main_data].catalog_id != '' AND ".implode(" AND ", $search_conditions).$sort_by_qry.$items_limit;
...
//        $items_prices = &cw_session_register('items_prices', array());
        foreach ($found_items as $item) {
            $found_items_alus[] = $item['catalog_id'];   
        }
