...
db_query("update cw_datahub_main_data dmd set initial_xref='' where initial_xref in (select xref from item_xref) and initial_xref NOT IN (select xref from item_xref where item_id=dmd.catalog_id)");
cw_datahub_filltemp_feeds_item_compare();
}

function dh_dbg(){
    cw_log_add(__FUNCTION__, cw_query_first("select ccode,sku,catalogid, avail_code,hide,cstock,pother1,name from xfer_products_SWE where catalogid=18200"));

//cw_log_add(__FUNCTION__,cw_query_first("select catalogid, ccode, Cost, cstock, sku from xfer_products_SWE where catalogid=788340"));
}

function dh_dbg2(){
cw_log_add(__FUNCTION__,cw_query_first("select * from pos where `Custom Field 5`='WDMN-2514046-14'"));
}
...
db_query("UPDATE IGNORE pos qs INNER JOIN xfer_temp xt ON xt.catalogid=qs.`Alternate Lookup` INNER JOIN item_xref ix ON ix.xref=xt.xref SET qs.`Custom Field 5`=xt.xref, qs.`Average Unit Cost`=ix.cost_per_bottle+ix.split_case_charge");
cw_log_add('dh_dbg', cw_query_first("select * from pos where `Item Number`=151248"));
db_query("UPDATE SWE_store_feed sf INNER JOIN pos p ON p.`Item Number`=sf.sku SET sf.cost=p.`Average Unit Cost`");
cw_log_add('dh_dbg', cw_query_first("select * from SWE_store_feed where sku=151248"));
*/
cw_flush(". ");
item_price::pricing_calc_SWE();
cw_log_add('dh_dbg', cw_query_first("select * from item_price where item_id = 788340"));
cw_flush(". ");
...
cw_flush(". ");
*/
//db_query("UPDATE IGNORE pos qs INNER JOIN xfer_temp xt ON xt.xref!=qs.`Custom Field 5` AND xt.catalogid=qs.`Alternate Lookup` SET qs.`Custom Field 5`=xt.xref, qs.`Average Unit Cost`=xt.cost");
//*/
...
cw_flush(". ");
xfer_products_SWE::set_supplierid();dh_dbg();
cw_flush(". ");
...
//check if there are lost items
    ...
    cw_csvxc_logged_query("INSERT INTO cw_datahub_pos_lost_items (catalogid, possible_sku) SELECT xps.catalogid, is2.store_sku FROM xfer_products_SWE xps LEFT JOIN item_store2 is2 ON is2.item_id=xps.catalogid WHERE xps.catalogid NOT IN (select CAST(`Alternate Lookup` as signed) FROM pos) AND xps.hide=0");
