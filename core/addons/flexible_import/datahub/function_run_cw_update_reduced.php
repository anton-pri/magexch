...
function datahub_import_run_cw_update_reduced($dh_update_step, $is_web_mode=false) {
...
//do not enable new items if they would be added with this reduced import
cw_csvxc_logged_query("UPDATE xfer_products_SWE SET hide=1 WHERE catalogid NOT IN (SELECT product_id FROM cw_products)");
...
foreach($all_fi_profiles as $fi_prof) {
    if ($fi_prof['dbtable_src'] == 'xfer_products_SWE' && $fi_prof['import_src_type'] == 'T' && $fi_prof['name']=='DataHub2CW Reduced')
...
if (!empty($cw_import_profile)) {
    ...
    $fi_run_msg = "Reduced Import profile '<b>$cw_import_profile[name]</b>' has been run. Reduced data is copied from hub to CW.";
...
//db_query("update $tables[products] p inner join cw_datahub_main_data dm on dm.catalog_id = p.product_id and dm.catalog_id != dm.ID and dm.catalog_id != 0 set p.status=0");
    ...
    if ($is_web_mode) {
        if ($config['flexible_import']['fi_manual_transition'] == "Y") {
            print("<h3>Import into CW tables is done...</h3><a href='index.php?target=datahub_transfer_after_import_reduced'>Run After Import Script</a>");
        } else {
            cw_header_location('index.php?target=datahub_transfer_after_import_reduced');
        }
    } else {
        return -1;
    }
