...
//                db_query("DELETE FROM $tables[datahub_buffer_match_config] WHERE mfield='$mfield'");
    ...
    } elseif ($action == 'save_update_nonstock_config') {
        db_query("DELETE FROM $tables[datahub_update_nonstock_config]");
        db_query("DROP TABLE IF EXISTS $tables[datahub_update_nonstock_preview]");
        foreach ($buffer_update_nonstock as $bfield => $update_nonstock_cfg) {

            if (empty($update_nonstock_cfg['mfield'])) {
                continue;
            }

            $update_nonstock_cfg['bfield'] = $bfield;

            cw_array2insert('datahub_update_nonstock_config', $update_nonstock_cfg, true);

        }
        cw_header_location("index.php?target=datahub_configuration#buff_update_nonstock");
...
$smarty->assign('update_nonstock_config', $update_nonstock_config);
