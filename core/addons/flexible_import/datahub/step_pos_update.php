<?php
cw_datahub_delay_autoupdate("target=datahub_step_pos_update");
...
db_query("UPDATE (item_store2 AS si INNER JOIN pos AS q ON si.store_sku = q.`Item Number`) INNER JOIN item_price AS pr ON si.item_id = pr.item_id and pr.store_id = 1 SET q.`MSRP` = pr.price WHERE si.store_id = 1");
