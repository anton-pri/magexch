...
*/
	...
	Function OutputDebug($message)
	{
		if($this->debug)
		{
			...
			error_log($message);
            cw_log_add('mslive', array('oauth client message'=>$message)); 
		}
		return(true);
	}
	...
	{
        $mslive_login_session = &cw_session_register('mslive_login_session', array());
		if(IsSet($mslive_login_session['OAUTH_STATE']))
			$state = $mslive_login_session['OAUTH_STATE'];
		else
			$state = $mslive_login_session['OAUTH_STATE'] = time().'-'.substr(md5(rand().time()), 0, 6);
		return(true);
	}
...
cw_log_add('mslive', array('state'=>$state));
		return(true);
	}
...
cw_log_add('mslive', array('code'=>$code));
		return(true);
	}
...
*/
	...
	{
        cw_log_add('mslive', array('stored access_token'=>$access_token));

        $mslive_login_session = &cw_session_register('mslive_login_session', array()); 
		$mslive_login_session['OAUTH_ACCESS_TOKEN'] = array(
...
*/
	Function GetAccessToken(&$access_token)
	{
        ...
        $mslive_login_session = &cw_session_register('mslive_login_session', array());
		if(IsSet($mslive_login_session['OAUTH_ACCESS_TOKEN'][$this->access_token_url]))
			$access_token = $mslive_login_session['OAUTH_ACCESS_TOKEN'][$this->access_token_url];
		else
			$access_token = array();

        cw_log_add('mslive', array('get access_token'=>$access_token));
		return true;
	}
/*
{metadocument}
...
*/
	Function ResetAccessToken()
	{
        ...
        $mslive_login_session = &cw_session_register('mslive_login_session', array()); 
		if(IsSet($mslive_login_session['OAUTH_ACCESS_TOKEN'][$this->access_token_url]))
			Unset($mslive_login_session['OAUTH_ACCESS_TOKEN'][$this->access_token_url]);
		return true;
	}
/*
{metadocument}
