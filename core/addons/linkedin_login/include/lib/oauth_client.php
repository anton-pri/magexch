...
*/
	...
	Function GetStoredState(&$state)
	{
		...
		if(IsSet($linkedin_login_session['OAUTH_STATE']))
			$state = $linkedin_login_session['OAUTH_STATE'];
		else
			$state = $linkedin_login_session['OAUTH_STATE'] = time().'-'.substr(md5(rand().time()), 0, 6);
		return(true);
	}
...
*/
	...
	{
        $linkedin_login_session = &cw_session_register('linkedin_login_session', array()); 
		$linkedin_login_session['OAUTH_ACCESS_TOKEN'] = array(
...
*/
	Function GetAccessToken(&$access_token)
	{
		...
		if(IsSet($linkedin_login_session['OAUTH_ACCESS_TOKEN'][$this->access_token_url]))
			$access_token = $linkedin_login_session['OAUTH_ACCESS_TOKEN'][$this->access_token_url];
		else
			$access_token = array();
