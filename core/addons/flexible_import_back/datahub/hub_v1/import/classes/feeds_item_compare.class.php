...
//						WHERE c.add_to_hub=true and isnull(c.catalogid) and c.Source <> 'Hub'";
		...
		$sql = "INSERT INTO item ( TareWeight, Vintage, Producer, `Size`, varietal, initial_xref, name, region, country, appellation, `sub-appellation`, `RP Rating`, `RP Review`, `WS Rating`, `WS Review`, `ST Rating`, `ST Review`, LongDesc, bot_per_case, dhmd_id)
						SELECT IF(c.size=\"6.0Ltr\",24,IF(c.size=\"5.0Ltr\",20,IF(c.size=\"3.0Ltr\",12,IF(c.size=\"1.5Ltr\",6,IF(c.size=\"375ml\",1.5,IF(c.size=\"187ml\" or c.size=\"200ml\",.75,IF(c.size=\"500ml\",2,IF(c.size=\"15.0Ltr\",60,IF(c.size=\"1.0Ltr\",4,3))))))))), c.vintage, c.producer, c.size, c.varietal AS Expr11, CONCAT(c.source , c.xref), c.Name, c.region, c.country, c.appellation, c.`sub-appellation`, c.Parker_rating, c.Parker_review, c.Spectator_rating, c.Spectator_review, c.Tanzer_rating, c.Tanzer_review, c.description, c.bottles_per_case, c.dhmd_id 
						FROM feeds_item_compare AS c
