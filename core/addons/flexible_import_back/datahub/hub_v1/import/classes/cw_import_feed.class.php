...
*/	
	...
	public static function update_item_xref() {
			...
			$sql = "UPDATE (item_xref AS i 
                      INNER JOIN cw_import_feed AS u ON i.xref=CONCAT('CW',u.feed_short_name,'-',Cast(u.ITEMID as CHAR),'-',Right(u.Vintage,2)))
	...
	public static function hub_apply_splitcase_to_cost_cw_import() {
		$sql = "UPDATE item_xref ixrf SET split_case_charge = (SELECT charge FROM splitcase_charges WHERE CONCAT('CW',company) = split_string(ixrf.xref,'-',1) limit 1) WHERE xref like 'CW%'";
		$result = mysql_query($sql) or sql_error($sql);			
	}
...
u.Vintage,
u.`size`,
CONCAT("CW",u.feed_short_name,"-",Cast(u.ITEMID as CHAR),"-",Right(u.Vintage,2)),
NULL,
u.bottles_per_case,
u.cost,
u.country,
...
u.qty_in_stock,
u.supplier_id 
FROM cw_import_feed AS u LEFT JOIN item_xref AS i ON CONCAT("CW",u.feed_short_name,"-",Cast(u.ITEMID as CHAR),"-",Right(u.Vintage,2))=i.xref
