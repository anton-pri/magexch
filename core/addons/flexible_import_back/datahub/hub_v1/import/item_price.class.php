...
//Equivalent:  pricing_calc_SWE	
	public static function pricing_calc_SWE() {
		...
		$sql = "Delete from item_price_twelve_bottle";
 		mysql_query($sql) or sql_error($sql);	
		...
		$sql = "UPDATE (item_price_twelve_bottle AS p LEFT JOIN item_store2 AS si ON p.item_id = si.item_id and si.store_id = '1') 
						LEFT JOIN SWE_store_feed AS sf ON sf.sku = si.store_sku 
		...
		$sql = "UPDATE item_price_twelve_bottle
						SET price = IF(price <= 0, price,  CAST(price as DECIMAL(10,2)) - (CAST(price as DECIMAL(10,2)) * 0.03))";
 		mysql_query($sql) or sql_error($sql);						
