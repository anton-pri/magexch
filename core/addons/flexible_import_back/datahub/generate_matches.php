    ...
    foreach ($b_words as $wrd) {
        ...
        if (!isset($b_words_weights[$wrd])) {
            if (!isset($analysed_words[$wrd])) {
                $b_words_weights[$wrd] = cw_query_first_cell("select ln($all_items_count/count(*)) from $tables[datahub_match_search_cache] where words like '% $wrd %' or words like '$wrd %' or words like '% $wrd'");
