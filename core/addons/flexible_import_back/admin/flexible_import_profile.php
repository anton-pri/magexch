...
$prefilled_data = &cw_session_register('prefilled_data');
        ...
        foreach ($mapping as $tbl_name => $tbl_fields) {
            ...
            if (!empty($mapping_fields2save[$tbl_name]) && !empty($map_process[$tbl_name])) {
                ...
                if (!empty($map_process2save[$tbl_name]['post_sql']))
                    $map_process2save[$tbl_name]['post_sql'] = base64_encode(str_ireplace(array('ALTER', 'CREATE TABLE', 'DROP', 'INSERT', 'REPLACE INTO'),'',$map_process2save[$tbl_name]['post_sql'])); 
