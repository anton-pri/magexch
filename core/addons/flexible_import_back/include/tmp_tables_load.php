...
$src_images_path = $app_dir."/../magexch/images";
...
$current_customer_id = $user_account['customer_id'];
...
//        cw_csvxc_logged_query("UPDATE tmp_load_CATEGORIES SET AVAIL=IF(tmp_load_CATEGORIES.AVAIL='Y',1,0)");
    ...
    foreach ($cat_ids as $c_id) {
        ...
        $cat_path = implode($categories_separator, $cat_names);
...
//add lvl field to tmp_cw_categories
    ...
    for ($i = $cats_level_max; $i>1; $i--) {
        $lvl = $i-1;
        $lvl_parent = $i-2;
        cw_csvxc_logged_query("UPDATE tmp_cw_categories ctc SET ref_parent_id = (SELECT category_id FROM tmp_cw_categories_2 WHERE lvl=$lvl_parent AND ctc.category_path LIKE CONCAT(category_path,'$categories_separator%') limit 1) WHERE lvl=$lvl");
    }

    cw_csvxc_logged_query("ALTER TABLE tmp_cw_categories ADD COLUMN new_category_name VARCHAR(255) NOT NULL DEFAULT ''");

    cw_csvxc_logged_query("UPDATE tmp_cw_categories ctc SET new_category_name = REPLACE(category_path, CONCAT((SELECT category_path FROM tmp_cw_categories_2 WHERE ctc.ref_parent_id = category_id limit 1),'".$categories_separator."'), '') where ctc.lvl > 0");
    ...
    $tlc_columns = cw_csvxc_get_table_fields('tmp_load_CATEGORIES');

    cw_csvxc_logged_query("ALTER TABLE tmp_load_CATEGORIES ADD COLUMN ref_is_new int(1) NOT NULL DEFAULT 1");

    foreach ($tlc_columns as $ef_colname) {
        if (in_array($ef_colname, array("ref_category_id","CATEGORYID", "CATEGORY", "DESCR", "META_DESCR", "AVAIL", "ORDERBY", "META_KEYWORDS", "ICON"))) continue;

        $attr_info = cw_query_first("SELECT * FROM cw_attributes WHERE field='$ef_colname' AND item_type='C' AND addon in ('', 'custom_magazineexchange')");
        $attr_id = $attr_info['attribute_id'];
        if ($attr_id) {
            if ($attr_info['type'] == 'selectbox') {
                $id_ef_colname = $ef_colname."_id";
                cw_csvxc_logged_query("ALTER TABLE tmp_load_CATEGORIES ADD COLUMN `$id_ef_colname` int(11) NOT NULL DEFAULT 0");

                cw_csvxc_logged_query("UPDATE tmp_load_CATEGORIES tlc, cw_attributes_default cad SET tlc.`$id_ef_colname`=cad.attribute_value_id WHERE cad.attribute_id='$attr_id' AND cad.value=tlc.`$ef_colname`");
                cw_csvxc_logged_query("INSERT INTO cw_attributes_default (value, attribute_id, active, facet) SELECT distinct(`$ef_colname`), $attr_id, 1, $attr_info[facet] FROM tmp_load_CATEGORIES WHERE `$id_ef_colname`=0");
                cw_csvxc_logged_query("UPDATE tmp_load_CATEGORIES tlc, cw_attributes_default cad SET tlc.`$id_ef_colname`=cad.attribute_value_id WHERE cad.attribute_id='$attr_id' AND cad.value=tlc.`$ef_colname` AND tlc.`$id_ef_colname`=0");
                cw_csvxc_logged_query("UPDATE tmp_load_CATEGORIES tlc, cw_attributes_values cav SET tlc.ref_is_new=0 WHERE tlc.CATEGORYID=cav.item_id AND cav.attribute_id='$attr_id' AND cav.item_type='C'", "$ef_colname ");
                cw_csvxc_logged_query("UPDATE cw_attributes_values cav, tmp_load_CATEGORIES tlc SET cav.value=tlc.`$id_ef_colname` WHERE tlc.ref_is_new=0 AND tlc.CATEGORYID=cav.item_id AND cav.item_type='C' AND cav.attribute_id='$attr_id'", ' ');
                cw_csvxc_logged_query("INSERT INTO cw_attributes_values (item_id, attribute_id, value, code, item_type) SELECT CATEGORYID, $attr_id, `$id_ef_colname`, 'EN', 'C' FROM tmp_load_CATEGORIES WHERE ref_is_new=1", ' ');
                 cw_csvxc_logged_query("ALTER TABLE tmp_load_CATEGORIES DROP COLUMN `$id_ef_colname`");
            } else {
                cw_csvxc_logged_query("UPDATE tmp_load_CATEGORIES tlc, cw_attributes_values cav SET tlc.ref_is_new=0 WHERE tlc.CATEGORYID=cav.item_id AND cav.attribute_id='$attr_id' AND cav.item_type='C'", "$ef_colname ");
                cw_csvxc_logged_query("UPDATE cw_attributes_values cav, tmp_load_CATEGORIES tlc SET cav.value=tlc.`$ef_colname` WHERE tlc.ref_is_new=0 AND tlc.CATEGORYID=cav.item_id AND cav.item_type='C' AND cav.attribute_id='$attr_id'", ' ');
                cw_csvxc_logged_query("INSERT INTO cw_attributes_values (item_id, attribute_id, value, code, item_type) SELECT CATEGORYID, $attr_id, `$ef_colname`, 'EN', 'C' FROM tmp_load_CATEGORIES WHERE ref_is_new=1", ' ');
            } 
            cw_csvxc_logged_query("UPDATE tmp_load_CATEGORIES SET ref_is_new=1", ' ');
        }
    }
    cw_csvxc_logged_query("ALTER TABLE tmp_load_CATEGORIES DROP COLUMN ref_is_new");
...
//        cw_csvxc_update_linked_table('tmp_load_PRODUCTS', 'MANUFACTURERID'); 
    ...
    $tlp_columns = cw_csvxc_get_table_fields('tmp_load_PRODUCTS');

    cw_csvxc_logged_query("ALTER TABLE tmp_load_PRODUCTS ADD COLUMN ref_is_new int(1) NOT NULL DEFAULT 1");

    foreach ($tlp_columns as $ef_colname) {
        if (in_array($ef_colname, array('id_products','PRODUCTID','PRODUCTCODE','PRODUCT','WEIGHT','LIST_PRICE','DESCR','FULLDESCR','KEYWORDS','AVAIL','RATING','FORSALE','SHIPPING_FREIGHT','FREE_SHIPPING', 'DISCOUNT_AVAIL','MIN_AMOUNT','DIM_X','DIM_Y','DIM_Z','LOW_AVAIL_LIMIT','FREE_TAX','CATEGORYID','CATEGORY','MEMBERSHIP','PRICE','THUMBNAIL','IMAGE','TAXES','ADD_DATE','MANUFACTURERID','MANUFACTURER','MEMBERSHIPID','SUPPLIERID','COST'))) continue;

        $attr_info = cw_query_first("SELECT * FROM cw_attributes WHERE field='$ef_colname' AND item_type='P' AND addon in ('', 'custom_magazineexchange')");
        $attr_id = $attr_info['attribute_id'];
        if ($attr_id) {
            if ($attr_info['type'] == 'selectbox') {
                $id_ef_colname = $ef_colname."_id";
                cw_csvxc_logged_query("ALTER TABLE tmp_load_PRODUCTS ADD COLUMN `$id_ef_colname` int(11) NOT NULL DEFAULT 0");

                cw_csvxc_logged_query("UPDATE tmp_load_PRODUCTS tlp, cw_attributes_default cad SET tlp.`$id_ef_colname`=cad.attribute_value_id WHERE cad.attribute_id='$attr_id' AND cad.value=tlp.`$ef_colname`");
                cw_csvxc_logged_query("INSERT INTO cw_attributes_default (value, attribute_id, active, facet) SELECT distinct(`$ef_colname`), $attr_id, 1, $attr_info[facet] FROM tmp_load_PRODUCTS WHERE `$id_ef_colname`=0");
                cw_csvxc_logged_query("UPDATE tmp_load_PRODUCTS tlp, cw_attributes_default cad SET tlp.`$id_ef_colname`=cad.attribute_value_id WHERE cad.attribute_id='$attr_id' AND cad.value=tlp.`$ef_colname` AND tlp.`$id_ef_colname`=0");
                cw_csvxc_logged_query("UPDATE tmp_load_PRODUCTS tlp, cw_attributes_values cav SET tlp.ref_is_new=0 WHERE tlp.PRODUCTID=cav.item_id AND cav.attribute_id='$attr_id' AND cav.item_type='P'", "$ef_colname ");
                cw_csvxc_logged_query("UPDATE cw_attributes_values cav, tmp_load_PRODUCTS tlp SET cav.value=tlp.`$id_ef_colname` WHERE tlp.ref_is_new=0 AND tlp.PRODUCTID=cav.item_id AND cav.item_type='P' AND cav.attribute_id='$attr_id'", ' ');
                cw_csvxc_logged_query("INSERT INTO cw_attributes_values (item_id, attribute_id, value, code, item_type) SELECT PRODUCTID, $attr_id, `$id_ef_colname`, 'EN', 'P' FROM tmp_load_PRODUCTS WHERE ref_is_new=1", ' ');
                cw_csvxc_logged_query("ALTER TABLE tmp_load_PRODUCTS DROP COLUMN `$id_ef_colname`");
            } else {
                cw_csvxc_logged_query("UPDATE tmp_load_PRODUCTS tlp, cw_attributes_values cav SET tlp.ref_is_new=0 WHERE tlp.PRODUCTID=cav.item_id AND cav.attribute_id='$attr_id' AND cav.item_type='P'", "$ef_colname ");
                cw_csvxc_logged_query("UPDATE cw_attributes_values cav, tmp_load_PRODUCTS tlp SET cav.value=tlp.`$ef_colname` WHERE tlp.ref_is_new=0 AND tlp.PRODUCTID=cav.item_id AND cav.item_type='P' AND cav.attribute_id='$attr_id'", ' ');
                cw_csvxc_logged_query("INSERT INTO cw_attributes_values (item_id, attribute_id, value, code, item_type) SELECT PRODUCTID, $attr_id, `$ef_colname`, 'EN', 'P' FROM tmp_load_PRODUCTS WHERE ref_is_new=1", ' ');
            }
            cw_csvxc_logged_query("UPDATE tmp_load_PRODUCTS SET ref_is_new=1", ' ');
        }
    }
    cw_csvxc_logged_query("ALTER TABLE tmp_load_PRODUCTS DROP COLUMN ref_is_new");
}
...
if (cw_csvxc_is_table_exists('tmp_load_PRODUCTS_EXTRA_FIELD_VALUES') && $csvxc_allowed_sections['tmp_load_PRODUCTS_EXTRA_FIELD_VALUES']) {
    ...
    if ($csvxc_fmap['tmp_load_PRODUCTS_EXTRA_FIELD_VALUES']['PRODUCTID']['exists_column']) {
        ...
        foreach ($efv_columns as $ef_colname) {
            ...
            $attr_info = cw_query_first("SELECT * FROM $fi_tables[attributes] WHERE field='$ef_colname' AND item_type='P' AND addon in ('', 'custom_saratogawine_magazines','custom_saratogawine_backorder', 'clean_urls')");
            $attr_id = $attr_info['attribute_id'];
            if ($attr_id) {
                if ($attr_info['type'] == 'selectbox') {
                    $id_ef_colname = $ef_colname."_id";
...
//    cw_csvxc_logged_query("UPDATE tmp_load_DETAILED_IMAGES SET IMAGE=REPLACE(IMAGE,'http://eplanters.com/images', '')");
    ...
    cw_csvxc_transfer_import_images('tmp_load_DETAILED_IMAGES', 'IMAGE', 'PRODUCTID');
}
