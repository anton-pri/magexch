REPLACE INTO `cw_addons` (`addon`, `descr`, `active`) VALUES ('order_messages', 'Allows to perform communication between store admin and customers or suppliers', '1');
...
SELECT @config_category_id:=config_category_id FROM cw_config_categories WHERE category = 'order_messages' AND is_main = 0;
...
replace into cw_config set name='default_recepient_admin_email', comment='Default admin who receives order messages customers replies', config_category_id=@config_category_id, orderby=40, type='text', value='';
